  ...
  group('AlertsCubit', () {
    ...
    group('loadAlerts', () {
      ...
      blocTest<AlertsCubit, AlertsState>(
        ...
        build: () => AlertsCubit(alertRepository: mockRepository),
        act: (cubit) => cubit.loadAlerts(),
        expect: () => [isA<AlertsLoading>(), isA<AlertsError>()],
      );
    });
    ...
    group('loadUnreadCount', () {
      blocTest<AlertsCubit, AlertsState>(
        ...
        expect: () => [
          isA<AlertsUnreadCountLoaded>().having((s) => s.count, 'count', 5),
        ],
      );
    ...
    group('markAsRead', () {
      blocTest<AlertsCubit, AlertsState>(
        ...
        build: () => AlertsCubit(alertRepository: mockRepository),
        seed: () => AlertsLoaded(alerts: testAlerts, unreadCount: 1),
        act: (cubit) => cubit.markAsRead(1),
        expect: () => [
          isA<AlertsLoaded>()
              .having((s) => s.alerts.first.isRead, 'first alert isRead', true)
    ...
    group('markAllAsRead', () {
      blocTest<AlertsCubit, AlertsState>(
        ...
        expect: () => [
          isA<AlertsLoaded>()
              .having((s) => s.alerts.every((a) => a.isRead), 'all read', true)
              .having((s) => s.unreadCount, 'unreadCount', 0),
        ],
      );
    });
